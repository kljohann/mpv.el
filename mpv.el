                   ...
                   (and (not (equal title 'false))
                        (if (stringp title)
                            (if (<= (length title) (1+ mpv-display-title-truncate-threshold))
                                (concat title " ")
                              (let* ((shortened-title (substring title 0 mpv-display-title-truncate-threshold))
                                     (last-char (aref shortened-title (- (length shortened-title) 1))))
                                (cond
                                 ((= last-char ?.) (concat shortened-title ".. "))
                                 ((= last-char ?\s)
                                  (if (= (aref shortened-title (- (length shortened-title) 2)) ?.)
                                      (concat (substring title 0 (- mpv-display-title-truncate-threshold 1)) ".. ")
                                    (concat (substring title 0 (- mpv-display-title-truncate-threshold 1)) "... ")))
                                 (t (concat shortened-title "... ")))))
